...
export default function InventoryPage() {
  ...
  function handleImageDrop(e: React.DragEvent) {
    ...
    if (file) {
      ...
      handleImageSelect({ ...fakeEvent, target: { files: e.dataTransfer.files } } as React.ChangeEvent<HTMLInputElement>);
    }
  }
  ...
  async function handleSave() {
    ...
    if (editProduct) {
      ...
      // Insert without select to avoid RLS read-back issue, then fetch the new row
      ...
      // Fetch the newly created product's ID for image upload
      if (imageFile) {
        const { data: newest } = await supabase
          .from("products")
          .select("id")
          .eq("shop_id", shopId)
          .eq("sku", basePayload.sku)
          .order("created_at", { ascending: false })
          .limit(1)
          .single();

        if (newest) {
          const image_url = await uploadImage(newest.id);
          if (image_url) {
            await supabase.from("products").update({ image_url }).eq("id", newest.id);
          }
        }
      }
          ...
          <div className="sm:col-span-2">
            ...
            <div
              ...
              className={cn(
                ...
                imagePreview ? "border-primary/30 bg-primary/5" : "border-border hover:border-primary/40"
              )}
            >
              {imagePreview ? (
                ...
                <button
                  onClick={() => imageInputRef.current?.click()}
                  className="w-full py-6 flex flex-col items-center gap-2 text-muted-foreground"
                >
